...
if __name__ == '__main__':
	...
	FPS = 30
	'''
		Distance to line in road: ~0.025KM
	'''
	ROAD_DIST_MILES = 0.025

	'''
		Speed limit of urban freeway (50-65 KMPH)
	'''
					...
					try:
						...
						if trace_y <= Y_THRESH + 5 and trace_y >= Y_THRESH - 5 and not vehicle.passed:
							...
							if vehicle.mph > HIGHWAY_SPEED_LIMIT:
								...
								cv2.putText(orig_frame, 'KMPH: %s' % int(vehicle.mph), (int(trace_x), int(trace_y)), font, 1, (0, 0, 255), 1, cv2.LINE_AA)
						...
						if vehicle.passed:
							...
							cv2.putText(frame, 'KMPH: %s' % int(vehicle.mph), (int(trace_x), int(trace_y)), font, 1, (0, 255, 255), 1, cv2.LINE_AA)
						else:
